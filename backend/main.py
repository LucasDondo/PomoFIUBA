...
def students():
        ...
        students = [{"id": student.id, "name": student.name, "age": student.age, "course_id": student.course_id} for student in response]
...
def new_student():
    try:
        ...
        if not name or not age or not course_id:
            return jsonify({"message": "Bad request: student name, age, and course_id must be provided."}), 400
        ...
        db.session.commit()
        return jsonify({"student": {"id": new_student.id, "name": new_student.name, "age": new_student.age, "course_id": new_student.course_id}}), 201
...
def update_student(id):
    try:
        ...
        if student:
            ...
            db.session.commit()
            return jsonify({"student": {"id": student.id, "name": student.name, "age": student.age, "course_id": student.course_id}}), 200
        else:
            return jsonify({"message": f"Student with ID {id} does not exist."}), 404
    except Exception as e:
        return jsonify({"message": str(e)}), 500
