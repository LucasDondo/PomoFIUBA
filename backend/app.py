...
def new_session():
    try:
        ...
        new_session = Session(course_id=course_id, mins_studied=mins_studied)
...
@app.delete("/delete_session_<int:id>")
def delete_session(id):
    try:
        ...
        if session:
            ...
            db.session.commit()
            return (
                jsonify({"message": f"Session with ID {id} deleted successfully."}),
                200,
            )
        else:
